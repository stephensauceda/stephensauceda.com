...
export function getFirstImageUrl(note) {
  return note.data.body.find(slice => slice.slice_type === 'image').primary
    .image.url
}
...
export function getTextSlices(slices) {
  return slices.filter(slice => slice.slice_type === 'text')
}

export function combineTextSlices(slices) {
  return slices.map(slice => RichText.asText(slice.primary.text)).join(' ')
}
